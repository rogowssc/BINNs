...
retrain = False
dynamic = True
chi_type = 'constant' #change to match mydir below!
...
#mydir = '../models/covasim/2025-01-29_11-23-41' #piecewise
#mydir = '../models/covasim/2025-01-30_04-31-07' #sin
mydir = '../models/covasim/2025-02-03_11-08-41' #constant
#mydir = '../models/covasim/2025-02-10_13-22-17' #sin
...
for i in range(3):
    ...
    if i == 0:
        ax.scatter(y, x, res1, c='k', s=15, alpha=1)
        ax.plot_surface(Y, X, res, cmap=cm.coolwarm, alpha=0.5)
        ax.scatter(Y.reshape(-1), X.reshape(-1), res.reshape(-1), s=1, c='w')
        
    elif i == 1:
        
        ax.plot_surface(Z, X, res, cmap=cm.coolwarm, alpha=0.5)
        ax.scatter(Z.reshape(-1), X.reshape(-1), res.reshape(-1), s=1, c='w')
        ax.scatter(z, x, res1, c='k', s=15, alpha=1)
    else:
        #ax.plot_surface(Y, Z, res, cmap=cm.coolwarm, alpha=1)
        ax.scatter(z, y, res1, c='k', s=15, alpha=1)
        ...
        ax.scatter(Z.reshape(-1), Y.reshape(-1), res.reshape(-1), s=1, c='w') #switching axis
        ax.set_zlabel('$\eta$')
...
chi_min, chi_max = 0.05, params['eff_ub'] #for Constant
...
labels = ['S + A + Y', r'$h(t)$', '$\beta$']

###Beta from Data##
u_grid1 = np.stack([np.ravel(SAY), np.ravel(chi)], axis=1)
res1 = beta(u_grid1) * params['n_contacts'] # * u_grid[:, [1]] *
...
ax.scatter(SAY, chi, res1, c='k', s=15, alpha=1)
ax.plot_surface(X, Y, res, cmap=cm.coolwarm, alpha=0.5)
ax.scatter(X.reshape(-1), Y.reshape(-1), res.reshape(-1), s=1, c='w')
ax.set_xlabel(x_label)
ax.set_ylabel(y_label)
...
u_grid = np.stack([np.ravel(data[:,3]), np.ravel(data[:,4])], axis=1)
res1 = tau(u_grid)
res1 = res1[:,0].reshape(y.shape)
res1 = tau_lb + (tau_ub - tau_lb) * res1 # scaling
res1 = np.round(res1, decimals=4)
...
fig = plt.figure(figsize=(10,7))
ax = fig.add_subplot(1, 1, 1, projection='3d')
ax.scatter(y, z, res1, c='k', s=15, alpha=1 )
ax.plot_surface(X, Y, res, cmap=cm.coolwarm, alpha=0.5)
ax.scatter(X.reshape(-1), Y.reshape(-1), res.reshape(-1), s=1, c='w')
ax.set_xlabel(x_label)
ax.set_ylabel(y_label)
